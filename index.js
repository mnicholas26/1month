...
function genErrors(str) {
    ...
    for (const c of str) {
        if (c === '@') slowline = true;
        if (c === '\n') slowline = false;
        let error = randomBetween(slowline ? 1000000 : errorChance) === 0;
        let errorCounter = 0;
        while (error) {
            ...
            if (neighbour !== undefined) {
                ...
                if (uppercase) newc = newc.toUpperCase();
                out += newc;
                errorCounter++;
            }
            error =
                randomBetween(
                    error ? Math.floor(errorChance / 100) : errorChance
                ) === 0;
        }
...
async function randomClack() {
    ...
    if (c === '{') currentp.textContent = currentp.textContent.slice(0, -1);
    else if (c === '#') return;
    else if (c === '~') return;
    else {
        clack();
